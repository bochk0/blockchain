...
class PasswordResetAPI(MethodView):
    ...
    def put(self, user_id):
        ...
        if user is not None:
            if reset_tfa and AccessControl.has_sufficient_tier(g.user.roles, 'ADMIN', 'admin'):
                user.reset_TFA()
            if reset_tfa and not AccessControl.has_sufficient_tier(g.user.roles, 'ADMIN', 'admin'):
                response_object = {
                    'status': 'fail',
                    'message': 'Only admininstrators may reset TFA codes.',
                }
